                    ...
                    <table border="0">
                        <tbody>
                            <tr>
                                ...
                                <td>CODIGO</td>
                                <td><input type="text" name="txtcodigocli" id="txtcodigo" ></td>
                                <td><button type="button" class="btn btn-success mb-2" onclick="buscarClientePorCedula()">BUSCAR</button></td>
...
<div class="container3">
    ...
    <label for="txtTotal"><strong>Total:</strong></label>
    <input type="text" id="txtTotal" name="total" readonly>

    <label for="txtPago"><strong>Pago del cliente:</strong></label>
    <input type="number" id="txtPago" name="pago" min="0" step="1">

    <label for="txtVuelto"><strong>Vuelto:</strong></label>
    <input type="text" id="txtVuelto" readonly>
</div>
...
// Asignar los datos a los inputs correspondientes
            ...
            function cargarproducto() {
                ...
                inputCantidad.style.color = "white";
...
// Eliminar la fila de la tabla
                ...
                calcularTotalYVuelto();
            }
            // Función para calcular total y mostrarlo
function calcularTotalYVuelto() {
    let total = 0;
    const filas = document.querySelectorAll("#tablaCuerpo tr");
    filas.forEach(fila => {
        const celdas = fila.getElementsByTagName("td");
        const exenta = parseFloat(celdas[4].textContent) || 0;
        const cinco = parseFloat(celdas[5].textContent) || 0;
        const diez = parseFloat(celdas[6].textContent) || 0;
        total += exenta + cinco + diez;
    });

    document.getElementById("txtTotal").value = total.toFixed(0); // asumimos guaraníes sin decimales
    calcularVuelto(); // recalcula vuelto automáticamente
}

// Función para calcular el vuelto
function calcularVuelto() {
    const total = parseFloat(document.getElementById("txtTotal").value) || 0;
    const pago = parseFloat(document.getElementById("txtPago").value) || 0;
    const vuelto = pago - total;
    document.getElementById("txtVuelto").value = vuelto >= 0 ? vuelto.toFixed(0) : "0";
}

// Agregar el evento de cambio en el input de pago
