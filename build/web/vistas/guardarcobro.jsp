...
%>
    ...
    let total = 0;
    // Seleccionamos TODOS los checkboxes con name facturasSeleccionadas
    const checkboxes = document.querySelectorAll('input[name="facturasSeleccionadas"]');

    checkboxes.forEach(checkbox => {
        if (checkbox.checked) {
            // El checkbox está dentro de la fila <tr>
            const fila = checkbox.closest("tr");
            if (fila) {
                // Tomamos la celda que contiene el monto, columna 3 (index 3)
                const celdas = fila.getElementsByTagName("td");
                const monto = parseFloat(celdas[3].textContent) || 0;
                total += monto;
            }
        }
    });
    ...
    // Calcula el vuelto en base al pago ingresado
    ...
    // Vincula el evento input para recalcular vuelto al escribir en el campo de pago
    document.addEventListener("DOMContentLoaded", function () {
    ...
    // Después de cargar las facturas, calcular el total automáticamente
    function obtenerFilacliente(boton) {
        event.preventDefault();
        var fila = boton.parentNode.parentNode;
        var celdas = fila.getElementsByTagName("td");
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                document.getElementById("tablaCuerpo").innerHTML = xhr.responseText;
                calcularTotalFacturas(); // calcular el total luego de cargar la tabla
            }
        };
